...
def generate_conv():
    ...
    for training_speaker in training_speakers:
        t = audio.define_random_mix(num_sentences=7, speaker_ids_to_choose_from=[training_speaker])
        training_targets.append(t)
    tr = audio.generate_mix(sum(training_targets, []))

    testing_targets = []
    for testing_speaker in testing_speakers:
        t = audio.define_random_mix(num_sentences=7, speaker_ids_to_choose_from=[testing_speaker])
